...
with open('src/pages/CandidateScores.jsx', 'r') as f:
    content = f.read()

# 1. Update imports
imports_old = """import React from 'react';
import { useParams, useNavigate } from 'react-router-dom';"""
imports_new = """import React, { useState, useEffect } from 'react';
import { useParams, useNavigate } from 'react-router-dom';"""
content = content.replace(imports_old, imports_new)

# 2. Add AI state variables and useEffect
useEffect_code = """
...
# 3. Add icons
content = content.replace('const { FiArrowLeft, FiCheckCircle, FiXCircle, FiMessageSquare, FiUser, FiAward, FiTarget, FiTrendingUp } = FiIcons;', 'const { FiArrowLeft, FiCheckCircle, FiXCircle, FiMessageSquare, FiUser, FiAward, FiTarget, FiTrendingUp, FiStar, FiCheck } = FiIcons;')

# 4. Insert UI after overall scores
ui_code_new = """
        {/* AI Assist Panel */}
...
content = content.replace('        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">', ui_code_new)

with open('src/pages/CandidateScores.jsx', 'w') as f:
    f.write(content)
