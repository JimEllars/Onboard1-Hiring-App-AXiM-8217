...
// .wrangler/tmp/pages-cIoPJI/functionsWorker-0.4127885638362385.mjs
...
var needsQuoting = /* @__PURE__ */ __name2((value) => PostgrestReservedCharsRegexp2.test(value) || value !== value.trim(), "needsQuoting");
var quote = /* @__PURE__ */ __name2((value) => `"${value.replace(/\\/g, "\\\\").replace(/"/g, '\\"')}"`, "quote");
var serializeScalar = /* @__PURE__ */ __name2((value) => {
  const serialized = value === null ? "null" : String(value);
  return needsQuoting(serialized) ? quote(serialized) : serialized;
}, "serializeScalar");
var serializeIsValue = /* @__PURE__ */ __name2((value) => value === null ? "null" : String(value), "serializeIsValue");
var serialize = /* @__PURE__ */ __name2((operator, value) => {
  if (operator === "in") {
...
var encodeStoragePath = /* @__PURE__ */ __name2((path) => path.split("/").map(encodeURIComponent).join("/"), "encodeStoragePath");
...
var pkceVerifierSlotKey = /* @__PURE__ */ __name2((storageKey, flowId) => `${storageKey}-flow-${flowId}-code-verifier`, "pkceVerifierSlotKey");
var pkceFlowIndexKey = /* @__PURE__ */ __name2((storageKey) => `${storageKey}-flows-code-verifier`, "pkceFlowIndexKey");
...
var isNewApiKey = /* @__PURE__ */ __name2((key) => key.startsWith("sb_publishable_") || key.startsWith("sb_secret_"), "isNewApiKey");
var TEMP_KEY_PREFIX = "sb_temp_";
var warnedKeySubtypes = /* @__PURE__ */ new Set();
var checkApiKeyFormat = /* @__PURE__ */ __name2((key) => {
...
var fetchWithAuth = /* @__PURE__ */ __name2((supabaseKey, supabaseUrl, getAccessToken, customFetch, tracePropagationOptions, options) => {
...
__name2(base64urlEncodeBuffer, "base64urlEncodeBuffer");
async function issueMagicLinkToken(candidateId, secret) {
  const header = {
    alg: "HS256",
    typ: "JWT"
  };
...
__name(issueMagicLinkToken, "issueMagicLinkToken");
__name2(issueMagicLinkToken, "issueMagicLinkToken");
...
async function verifyMagicLinkToken(token, secret) {
...
__name2(verifyMagicLinkToken, "verifyMagicLinkToken");
async function sendMagicLinkEmail(email, token, env, origin) {
...
async function onRequestOptions(context) {
  ...
  if (origin) {
    try {
      ...
      if (originUrl.hostname === "localhost" || originUrl.hostname === "127.0.0.1" || originUrl.hostname.endsWith(".pages.dev")) {
...
async function onRequestPost(context) {
  try {
    ...
    const { data: candidate, error: dbError } = await supabase.from("onboard1_candidates").insert({
...
async function onRequestGet(context) {
  ...
  try {
    ...
    if (env.SUPABASE_URL && env.SUPABASE_SERVICE_KEY) {
      const supabase = createClient(env.SUPABASE_URL, env.SUPABASE_SERVICE_KEY);
      const { data: candidate, error } = await supabase.from("onboard1_candidates").select("*").eq("id", payload.candidateId).single();
      if (!error && candidate) {
...
// .wrangler/tmp/bundle-M6qcQ3/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-M6qcQ3/middleware-loader.entry.ts
...
//# sourceMappingURL=functionsWorker-0.4127885638362385.js.map
